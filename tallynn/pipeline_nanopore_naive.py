...
@follows(mkdir("collapse.dir"))
@transform(correct_polyA,
           regex("polyA_correct.dir/(\S+)_correct_polya.fastq"),
           r"collapse.dir/\1_R1.fastq")
def naive_collapse(infile, outfile):
    '''
    Identify ambigous and unabiguous reads
    '''

    name = outfile.replace("_R1.fastq", "")

    PYTHON_ROOT = os.path.join(os.path.dirname(__file__), "python/")

    statement = '''python %(PYTHON_ROOT)s/naive_collapse.py --outname=%(name)s --infile=%(infile)s'''
